...
import socket, sys, time, datetime, random
...
def client(conn, addr):
    ...
    try:
        while True:
            ...
            # Send
            conn.sendall(str.encode(cs.status + " What is " + str(a) + " x " + str(b) + "?\n" + cs.status + " Input: "))
            ...
            # Process
            if int(data) == ans:
                conn.sendall(str.encode(cs.status + " That's correct!\n\n"))
            else:
                raise ServerError("giving a wrong answer")
